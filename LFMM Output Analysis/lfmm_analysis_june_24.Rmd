...
snp_table <- q.value.snp %>%
  mutate(chr_color = ifelse(as.numeric(CHROM_NUM) %% 2 == 0,"black", "azure4"),
         ...
         plot_color = ifelse(temp6_10_KB <= 0.01, "#0072B2",            # highly significant
                             ifelse(temp6_10_KB <= 0.05, "#E69F00", chr_color) ))
...
tmp <- manhattan_data_preprocess(
  snp_table, pval.colname = "temp6_10_KB", 
...
g <- manhattan_plot(
  tmp, plot.title = "Potential evapotranspiration PC1 Manhattan plot",
...
#Save the plots
png("/scratch/ib2382/GEA/June_lfmm_804/K6_10kb_pruning/pet_manhattan.png", width = 1200, height = 800, res = 150)
...
# table for supplemental material:
The following code creates a table that contains Chromosome, peak number, focal SNP position, name, q.value, peak start and end.

```{r}
evapotranspiration_annotations=read.csv("evapotranspiration_annotations-2.csv")
temperature_annotations=read.csv("temperature_annotations-3.csv")
precipitation_annotations=read.csv("precipitation_annotations-2.csv")
library(dplyr)
summary_table <- precipitation_annotations %>% 
  group_by(chain) %>%
  summarise(
    CHROM = CHROM,
    sig_SNP = sig_SNP,
    sig_SNP_name = sig_SNP_name,
    sig_SNP_POS = sig_SNP_POS,
    peak_start = peak_start,
    peak_end = peak_end,
    genes_in_peak = paste(
      unique(na.omit(trimws(Oryzabase.Gene.Symbol.Synonym.s.))),
      collapse = ", "
    ),
    TO_in_peak = paste(
      unique(na.omit(trimws(Trait_Oncology))),
      collapse = ", "
    ),
    .groups = "drop"
  ) %>%
  distinct()
write.csv(summary_table, "precipitation_summary_table.csv", row.names = F)
```
